...
async function scoreResume({ extractedData }) {
  ...
  const breakdown = {
    contactInfo: {
      score:
        (extractedData.name ? 0.4 : 0) +
        (extractedData.email ? 0.3 : 0) +
        (extractedData.phone ? 0.15 : 0) +
        (extractedData.location ? 0.15 : 0),
      max: 1.0,
    },
    summary: {
      score:
        (extractedData.summary || extractedData.generatedSummary || '').length > 150
          ? 1.0
          : (extractedData.summary || '').length > 50
            ? 0.7
            : 0,
      max: 1.0,
    },
        ...
        ((extractedData.skills || []).length >= 10
          ...
          : (extractedData.skills || []).length >= 6
            ...
            : (extractedData.skills || []).length >= 3
              ? 0.5
              : 0) + ((extractedData.softSkills || []).length > 0 ? 0.5 : 0),
        2.0
      ),
      max: 2.0,
    },
    workExperience: {
      score:
        ((extractedData.workExperience || []).length >= 2
          ? 1.0
          : (extractedData.workExperience || []).length > 0
            ? 0.5
            : 0) + ((extractedData.workExperience || []).some((e) => (e.responsibilities || '').length > 50) ? 1.0 : 0),
      max: 2.0,
    },
    education: {
      score: Math.min(
        ((extractedData.education || []).length >= 2 ? 1.0 : (extractedData.education || []).length > 0 ? 0.5 : 0) +
          ((extractedData.education || []).some((e) => e.degree && e.institution) ? 0.5 : 0),
        1.5
      ),
      max: 1.5,
    },
    ...
    extras: {
      score:
        ((extractedData.certificates || []).length > 0 ? 0.25 : 0) +
        ((extractedData.languages || []).length > 0 ? 0.25 : 0),
      max: 0.5,
    },
  };
  ...
  if ((extractedData.workExperience || []).some((e) => !e.responsibilities || e.responsibilities.length < 50))
    recommendations.push('Add detailed descriptions to work experience');
