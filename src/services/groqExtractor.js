...
async function extractResumeData(text) {
  ...
  console.log('🔑 GROQ_API_KEY present, prefix:', apiKey.substring(0, 8) + '...');
...
${text}`;
  ...
  try {
    ...
    if (err.response) {
      console.error(`❌ Groq API error ${err.response.status}:`, JSON.stringify(err.response.data, null, 2));
    } else if (err.code === 'ECONNABORTED') {
      console.error('❌ Groq API request timed out after 30s');
    } else {
