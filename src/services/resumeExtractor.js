...
function normalizeExtractedData(extractedData) {
  ...
  if (extractedData.years_experience !== undefined && !extractedData.yearsOfExperience)
    extractedData.yearsOfExperience = extractedData.years_experience;
  if (extractedData.soft_skills && !extractedData.softSkills) extractedData.softSkills = extractedData.soft_skills;
  if (extractedData.generated_summary && !extractedData.generatedSummary)
    extractedData.generatedSummary = extractedData.generated_summary;
  if (extractedData.industry && !extractedData.industryExperience)
    extractedData.industryExperience = [extractedData.industry];
  if (extractedData.certifications && !extractedData.certificates)
    extractedData.certificates = extractedData.certifications;
  if (extractedData.experience && !extractedData.workExperience)
    extractedData.workExperience = extractedData.experience;
  ...
  return {
    ...
    workExperience: normalizeEmbeddedArray(extractedData.workExperience, (value) => {
      ...
      if (typeof value === 'string') {
        ...
        return text
          ? { position: text, company: '', duration: '', location: '', responsibilities: '', contact: '' }
          : null;
      }
    ...
    achievements: normalizeEmbeddedArray(extractedData.achievements, (value) => {
      ...
      return typeof value === 'string'
        ? value.trim() || null
        : String(value.description || value.name || '').trim() || null;
    }),
  };
}
...
function extractNameEnhanced(lines, text) {
  ...
  for (let i = 0; i < Math.min(8, lines.length); i++) {
    ...
    if (words.length >= 2 && words.length <= 5) {
      const isValidName = words.every((word) => {
        ...
        return (
          cleanWord.length > 1 && /^[A-Z][a-z'-]*$/.test(cleanWord) && !commonWords.includes(cleanWord.toLowerCase())
        );
      });
      if (isValidName) {
  ...
  try {
    ...
    for (const person of people) {
      if (person.length > 4 && person.length < 50 && !commonWords.some((word) => person.toLowerCase().includes(word)))
        return person;
    }
  } catch (e) {
    /* ignore */
  ...
  if (headerMatch && headerMatch[1] && !commonWords.some((word) => headerMatch[1].toLowerCase().includes(word)))
...
function extractSkills(text) {
  ...
  const skillsSection = text.match(
    /(?:skills|technical skills|core competencies|technologies|expertise)[:\s]+([\s\S]*?)(?:\n\s*\n|experience|education|$)/i
  );
  ...
  if (skillLines) {
    skillLines.forEach((line) => {
      ...
      if (
        ...
        !cleanSkill.includes('@') &&
        !cleanSkill.includes('http')
      ) {
        if (
          techSkills.some(
            (tech) =>
              tech.toLowerCase().includes(cleanSkill.toLowerCase()) ||
...
function extractJobExperience(text) {
  ...
  const experienceSection = text.match(
    /(?:experience|work experience|employment|professional experience|career history)[:\s]+([\s\S]*?)(?:\n\s*\n|education|skills|$)/i
  );
  ...
  industryKeywords.forEach((industry) => {
    if (new RegExp(`\\b${industry}\\b`, 'gi').test(text)) industries.add(industry);
  });
...
function extractEducation(text) {
  ...
  const educationSection = text.match(
    /(?:education|academic background|qualifications|academic|university|college)[:\s]+([\s\S]*?)(?:\n\s*\n|experience|skills|$)/i
  );
...
function extractLanguages(text) {
  ...
  const languageSection = text.match(
    /(?:languages|language skills|linguistic)[:\s]+([\s\S]*?)(?:\n\s*\n|experience|skills|education|$)/i
  );
  const searchText = languageSection ? languageSection[1] : text;
  commonLanguages.forEach((lang) => {
    if (new RegExp(`\\b${lang}\\b`, 'gi').test(searchText)) languages.add(lang);
  });
...
function extractCertifications(text) {
  ...
  const certSection = text.match(
    /(?:certifications|certificates|credentials|professional development)[:\s]+([\s\S]*?)(?:\n\s*\n|experience|skills|education|$)/i
  );
...
function generateProfessionalSummary(data, _originalText) {
  try {
    if (data.summary && data.summary.length > 50)
      return data.summary.length > 800 ? data.summary.substring(0, 800) + '...' : data.summary;
    const parts = [];
...
function generateBasicSummary(data) {
  ...
  parts.push(
    data.skills && data.skills.length > 0
      ? `with expertise in ${data.skills.slice(0, 3).join(', ')}`
