...
async function retrieve({ domain, keywords, location, limit = 50, offset = 0 }) {
  logger.info(
    `jobRetrievalService: domain=${domain} keywords=[${(keywords || []).join(', ')}] location=${location} limit=${limit}`
  );
  ...
  if (searchTerms) {
    try {
      jobs = await JobListing.find({ ...filter, $text: { $search: searchTerms } }, { score: { $meta: 'textScore' } })
      ...
      const regexQueries = keywords
        .filter((k) => k.length > 2)
        .map((k) => ({
          $or: [{ title: { $regex: k, $options: 'i' } }, { description: { $regex: k, $options: 'i' } }],
        }));
    ...
    jobs = await JobListing.find(filter).sort({ createdAt: -1 }).skip(offset).limit(limit).lean();
  }
